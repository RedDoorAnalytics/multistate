...
`PS' predictms_merlin_setup_stand(`SGS' S, `RR' b, `RS' N, `RS' trans, | `RC' t0)
{
	strtrans 	= strofreal(trans)
	at 		= S.at
	std 		= S.std
...
// 	stata("cap set obs "+st_local("_tempobs"))
	...
	if (st_local("survsim")=="") {
		...
		for (i=1;i<=cols(zerovars);i++) {
			//stata("qui replace "+zerovars[i]+" = 0 if _n<="+strofreal(N))
			stata("qui replace "+zerovars[i]+" = 0")
		}
...
// 		}
		...
		if (!S.hasmodels) {
			...
			for (i=1;i<=S.Ntrans;i++) {
				...
				if (i==trans) 	stata("qui replace "+ti+" = 1")
				else 		stata("qui replace "+ti+" = 0")
				
				//rebuild any *_transi variables
				if (st_local("toupdate")!="") {
					for (k=1;k<=rows(S.toupdate);k++) {
						stata("cap replace "+S.toupdate[k]+ti+"= "+S.toupdate[k]+"*"+ti)
					}
				}
			}
