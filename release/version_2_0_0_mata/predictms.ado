...
program define predictms, sortpreserve properties(st) rclass
	...
	if ("`tscale2'"!="" & "`time2'"=="" ) | ("`tscale2'"=="" & "`time2'"!="" ) {
		di as error "timematrix() and t2variable() must both be specified"
		exit 198
	}
	...
	// models framework
			...
			forval a=1/`Nats' {
				...
				forvalues i=1/`Ntrans' {
					...
					//error checks
					capture estimates restore `modelests`i''
...
program predictms_modelcheck
	...
	if "`e(cmd)'"!="megenreg" & "`e(cmd)'"!="stms" & "`e(cmd)'"!="stpm2" & "`e(cmd)'"!="stgenreg" & "`e(cmd2)'"!="streg" /*& "`e(cmd)'"!="cox"*/ & "`e(cmd)'"!="strcs" {
