...
{pstd}
{cmd:msAJ} calculates the Aalen-Johansson estimates of the transition probabilities. Before using {cmd:msAJ} you should use {cmd:msset} and then {cmd:stset}. See the examlpe{p_end}
...
This means that it is not necessary to specifify the {opt transmatrix()} option.
...
{phang}{stata "line P_AJ_1* _t, connect(stepstair)":. line P_AJ_1* _t, connect(stepstair)}{p_end}

{pstd}Probability in State 2 (recurrance){p_end}
{phang}{stata "line P_AJ_2* _t, connect(stepstair)":. line P_AJ_2* _t, connect(stepstair)}{p_end}

{pstd}Probability in State 3 (dead){p_end}
{phang}{stata "line P_AJ_3* _t, connect(stepstair)":. line P_AJ_3* _t, connect(stepstair)}{p_end}
