...
program define predictms, sortpreserve properties(st) rclass
	...
	//create variables to hold trans probs		
	cap drop pred_*
	foreach fromstate in `from' {
		forvalues i=1/`Nstates' {
			qui gen double pred_`fromstate'_`i' = 0 if _n<= `obs'
			local fromvars`fromstate' `fromvars`fromstate'' pred_`fromstate'_`i'
			//!! add labels
			if "`los'"!="" {
				if "`at2'"=="" {
					label var pred_`fromstate'_`i' "Length of stay in state=`i', from state=`fromstate'"
				}
				else {
					if "`ratio'"!="" {
						label var pred_`fromstate'_`i' "LoS(at()) / LoS(at2()), in state=`i', from state=`fromstate'"
					}
					else {
						label var pred_`fromstate'_`i' "LoS(at()) - LoS(at2()), in state=`i', from state=`fromstate'"
					}
			...
			if "`los'"=="" {
				if "`at2'"=="" {
					label var pred_`fromstate'_`i' "Transition prob. state=`i', from state=`fromstate'"
				}
				else {
					if "`ratio'"=="" {
						label var pred_`fromstate'_`i' "Transition prob(at()) - Transition prob(at2()) state=`i', from state=`fromstate'"
					}
					else {
						label var pred_`fromstate'_`i' "Transition prob(at()) / Transition prob(at2()) state=`i', from state=`fromstate'"
					}
			...
			if "`ci'"!="" & "`aj'"=="" {
				if "`normal'"=="" {
					qui gen double pred_`fromstate'_`i'_lci = 0 if _n<= `obs'
					qui gen double pred_`fromstate'_`i'_uci = 0 if _n<= `obs'
					local fromvarslci`fromstate' `fromvarslci`fromstate'' pred_`fromstate'_`i'_lci
					local fromvarsuci`fromstate' `fromvarsuci`fromstate'' pred_`fromstate'_`i'_uci
				}
				else {
					tempvar pred_`fromstate'_`i'_se
					qui gen double `pred_`fromstate'_`i'_se' = 0 if _n<= `obs'
					local probvars_`fromstate'_`i' pred_`fromstate'_`i' `pred_`fromstate'_`i'_se'
				}
			}
			else if "`ci'"!="" & "`aj'"!="" {
				qui gen double se_pred_`fromstate'_`i' = 0 if _n<= `obs'
				local se_fromvars`fromstate' `se_fromvars`fromstate'' se_pred_`fromstate'_`i'
			}
		}	
	}
		...
		if "`ci'"!="" & "`normal'"!="" & "`aj'"=="" {	
			...
			foreach fromstate in `from' {
				...
				forvalues i=1/`Nstates' {
					...
					gen double pred_`fromstate'_`i'_lci = pred_`fromstate'_`i' - `siglev'* sqrt(`pred_`fromstate'_`i'_se') if _n<=`obs'
					gen double pred_`fromstate'_`i'_uci = pred_`fromstate'_`i' + `siglev'* sqrt(`pred_`fromstate'_`i'_se') if _n<=`obs'
					
					replace pred_`fromstate'_`i' = `func'(pred_`fromstate'_`i') if _n<=`obs'
					replace pred_`fromstate'_`i'_lci = `func'(pred_`fromstate'_`i'_lci) if _n<=`obs'
					replace pred_`fromstate'_`i'_uci = `func'(pred_`fromstate'_`i'_uci) if _n<=`obs'
	...
	if "`graph'"!="" {
		msgraph, from(`from') nstates(`Nstates') timevar(`timevar') enter(`enter') `graphopts'
	}
